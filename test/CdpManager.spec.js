...
import { ETH, MMCR, GNT, DGD, BAT } from '../src';
...
test('getCombinedDebtValue', async () => {
  ...
  await cdpMgr.openLockAndDraw('ETH-A', ETH(1), MMCR(3));
  await cdpMgr.openLockAndDraw('ETH-A', ETH(2), MMCR(5));
...
test('transaction tracking for openLockAndDraw', async () => {
  ...
  const open = cdpMgr.openLockAndDraw('ETH-A', ETH(1), MMCR(0));
  expect.assertions(5);
  const handlers = {
...
test('get event history via web3', async () => {
  await setupCollateral(maker, 'ETH-A', { price: 150, debtCeiling: 50 });
  const cdp = await cdpMgr.openLockAndDraw('ETH-A', ETH(1), MMCR(3));
