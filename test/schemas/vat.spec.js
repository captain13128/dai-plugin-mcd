...
test(DEBT_CEILING, async () => {
  ...
  expect(ethADebtCeiling.isEqual(MMCR(100000))).toEqual(true);
  expect(batADebtCeiling.isEqual(MMCR(5000))).toEqual(true);
});
...
test(TOTAL_DAI_SUPPLY, async () => {
  ...
  const ethADaiGenerated = MMCR.rad(BigNumber(ethAArt).times(ethARate));
  const batADaiGenerated = MMCR.rad(BigNumber(batAArt).times(batARate));
  ...
  expect(totalDaiAmount.symbol).toEqual('MMCR');
...
test(GLOBAL_DEBT_CEILING, async () => {
  ...
  expect(globalDebtCeiling.symbol).toEqual(MMCR.symbol);
